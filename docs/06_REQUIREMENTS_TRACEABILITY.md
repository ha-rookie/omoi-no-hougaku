...
| --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-001 | Google Maps共有から地点登録候補を自動確定 | APP-002, APP-004, APP-010, APP-020, APP-021, APP-033, APP-034, APP-035, IF-005〜IF-009 | ADR-0004 | #18 #20 #22 #25 #29 | PoC: `share-location-poc.html`, `share-location-resolver.mjs`, `functions/api/resolve-location.js` / 本番は未実装 | unit + Android実機統合PoC | ACTIVE |
| REQ-002 | 地点へ任意の表示名を設定 | APP-002, APP-011, DATA-001 | ADR-0002 | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-003 | 最大5地点を端末内保存 | APP-011, APP-030, DATA-001, DATA-002 | ADR-0002 | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-004 | 保存地点の一覧・選択・削除 | APP-001, APP-030 | ADR-0002 | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-005 | 現在地取得 | APP-031, IF-002 | - | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-006 | 現在地から目的地への初期方位角計算 | APP-022 | - | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-007 | 端末方位と目的地方向を表示 | APP-003, APP-012, APP-032, IF-003 | - | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-008 | Google Mapsを外部で開く | UI-005, IF-004 | ADR-0004 | 本番Implementation Issueで作成 | 未実装 | 未実装 | ACTIVE |
| REQ-009 | PWAとしてインストール | APP §11 | ADR-0004 | #18 #25 #27 #29 | PoC manifest/service worker / 本番未実装 | Android実機 + manifest validation | ACTIVE |
| REQ-010 | Google Maps共有先として受信 | APP-033, IF-005 | ADR-0004 | #18 #25 #27 #29 | PoC Web Share Target / 本番未実装 | Android実機統合PoC | ACTIVE |
| NFR-001 | 登録地点をサーバーDBへ保存しない | ARCH Security / APP-030 | ADR-0002, ADR-0004 | 全関連Issue | Resolver stateless / localStorage予定 | Network review | ACTIVE |
| NFR-002 | 外部入力を検証し未検証HTML挿入を避ける | APP-020, APP-021, APP §13 | ADR-0004 | #25 #29 | PoC classifier/Function validation | 異常系 + static review | ACTIVE |
| NFR-003 | 解析失敗時に推測座標を返さない | APP-010, APP-020, APP-021, APP-035 | ADR-0004 | #22 #25 #29 | PoC統合判定 | invalid/unsupported + Android実機 | ACTIVE |
...
| NFR-006 | モバイル回線でも軽量表示 | UI/App全体 | - | 本番Implementation Issue | 未実装 | Lighthouse等 | ACTIVE |
...
| NFR-010 | 任意ピンは外部APIへ送らず端末内で確定 | ARCH-005, APP-020, APP-021 | ADR-0004 | #25 #29 | PoC shared-title-coordinate | `apiCalled=false` Android実機 | ACTIVE |
